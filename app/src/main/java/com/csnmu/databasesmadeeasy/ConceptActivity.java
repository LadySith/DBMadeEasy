...
public class ConceptActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // TODO: Set ActionBar title to concept name



        //fill list of screens | list of content pages
        Concept thisConcept = returnConcept(conceptName);
        final List<Page> mList = Arrays.asList(thisConcept.getConceptPages());

        //setup viewpager
        screenPager = findViewById(R.id.vp_conceptPager);
        conceptViewPagerAdapter = new ConceptViewPagerAdapter(this, mList);
    ...
    private Concept returnConcept(String name){
        Concept getConcept = null;
        for (int i = 0; i < conceptsArray.size(); i++){
            if (((Concept)conceptsArray.get(i)).getConceptTitle().equals(name)){
    ...
    private void populateConcepts(){
        ...
        Page c1p6 = new Page(R.drawable.img12, "File-based System Critique","A File System may make data management difficult for any or all of the following reasons:\n" +
        ...
        Page c3p4 = new Page(R.drawable.img31, "Reducing Data Anomalies", "Ideally, a field value change should be made in only a single place. A data anomaly occurs when all of the required changes in the redundant data are not made successfully");
        Page c3p5 = new Page(R.drawable.img32,"Types of anomaly", "Insertion anomaly\n" +
                "To add each new customer in the CUSTOMER file, you must also add the corresponding agent data. If you enter several hundred new customers, you need to enter several hundred agent names and telephone numbers. The potential for creating data inconsistencies would be great.\nUpdate anomaly\n" +
        ...
        Page c3p6 = new Page("Enforcing Entity Integrity", "◦\tEach primary key value must be unique to ensure that each row is uniquely identified by the primary key. In that case, the table is said to exhibit entity integrity. \n" +
        ...
        Page c3p7 = new Page(R.drawable.img33, "Enforcing Entity Integrity", "Referential integrity implications:\n" +
        ...
        Page[] c3pages = new Page[]{c3p1, c3p2, c3p3, c3p4, c3p5, c3p6, c3p7};
        ...
        Page c4p3 = new Page(R.drawable.img41,"Unnormalised Database", "◦\tKeep track of puppies, kennels and puppy tricks\n" +
                "◦\tPuppies only belong to 1 kennel\nDatabase schema\n" +
                "PuppyTricks (P_Num<pk>, P_Name, K_Code, K_Name, K_Location, Tr_ID<pk>, Tr_Name, Tr_Level)\n");
        Page c4p4 = new Page(R.drawable.img41,"How do we fix this (i.e. normalise our tables)?", "");
        Page[] c4pages = new Page[]{c4p1,c4p2,c4p3,c4p4};
